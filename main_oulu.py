...
from load import list_to_tensor, load_oulu
...
def Train(epochs, learning_rate, batchsize, criterion, optmizer, model, device, i):
    ...
    for e in range(epochs):
        ...
        for ind_batch in range(num_batches_train):
            x = data_X_train[ind_batch * batchsize: (ind_batch + 1) * batchsize]
            y = data_Y_train[ind_batch * batchsize : (ind_batch + 1) * batchsize]
            data, labels = x.to(device), y.to(device).long()
            optmizer.zero_grad()
        ...
        for ind_batch in range(num_batches_val):
            x = data_X_val[ind_batch * batchsize: (ind_batch + 1) * batchsize]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cls', '--num_classes', type=int, default=6, help='number of classes')
    ...
    data_X, data_Y = load_oulu('data/Oulu')
    num_exp_class = 6
    ...
    for i in range(10):
        ...
        data_X_train, data_Y_train = list_to_tensor(data_X_train, data_Y_train, image_size)
        data_X_val, data_Y_val = list_to_tensor(data_X_val, data_Y_val, image_size)

        index = [j for j in range(len(data_X_train))]
        random.shuffle(index)
        data_X_train, data_Y_train = data_X_train[index], data_Y_train[index]

        num_classes = args.num_classes

        if args.network == 'DAtt_ViT':
            model = DAtt_ViT(pretrained=True, image_size=224, num_classes=num_classes)
        else:
